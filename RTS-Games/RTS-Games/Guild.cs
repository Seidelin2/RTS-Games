...
namespace RTS_Games
{
	class Guild : GameObject
	{
		...
		public Guild(string goldMine, Vector2 position, float layer)
		{
			guildName = goldMine;
		...
		{
			unactiveBuilding = content.Load<Texture2D>($"Sprites/Buildings/{guildName}");
			activeBuilding = content.Load<Texture2D>($"Sprites/Buildings/{guildName}_Outline");
