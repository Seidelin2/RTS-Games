...
namespace RTS_Games.Buildings
{
	class Farm : GameObject
	{
		...
		public Farm(string barnFarm, Vector2 position, float layer)
		{
			farmName = barnFarm;
		...
		{
			unactiveBuilding = content.Load<Texture2D>($"Sprites/Buildings/{farmName}");
			activeBuilding = content.Load<Texture2D>($"Sprites/Buildings/{farmName}_Outline");
